...
export default function TripCard({ trip, onClick, index = 0 }: TripCardProps) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn(
        "group flex items-start gap-4 p-5 rounded-[28px] bg-surface border border-border shadow-sm",
        "hover:-translate-y-0.5 hover:shadow-xl hover:border-amber-500/20 transition-all duration-300",
        ...
        <div className="mt-3 space-y-1.5">
          <div className="flex items-start gap-2 text-sm">
            ...
            <span className="text-text-primary truncate">
              {trip.pickupAddress.formatted}
            </span>
          </div>
          <div className="flex items-start gap-2 text-sm">
            <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 shrink-0" />
            <span className="text-text-primary truncate">
              {trip.dropoffAddress.formatted}
            </span>
          </div>
        </div>
