...
export default function BusinessCard({ business, countryCode = 'NG', href, index = 0 }: BusinessCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={linkHref}
        className="group block bg-surface border border-border rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-amber-500/5 hover:border-amber-500/30 transition-all duration-300"
      >
        <div className="relative h-44 bg-gradient-to-br from-amber-500/10 to-amber-600/10 flex items-center justify-center overflow-hidden">
          {business.media?.coverUrl ? (
            <img src={business.media.coverUrl} alt={business.name} className="w-full h-full object-cover" />
          ) : (
          ...
          <button
            ...
            className={cn(
              'absolute top-3 right-3 p-2 rounded-lg backdrop-blur-sm transition-all z-10',
              isFav ? 'bg-red-500 text-white' : 'bg-white/80 text-text-secondary hover:text-red-500'
            )}
          >
        ...
        <div className="p-5">
          ...
          {business.address?.formatted && (
            <p className="mt-1.5 text-xs text-text-tertiary truncate">{business.address.formatted}</p>
          )}
