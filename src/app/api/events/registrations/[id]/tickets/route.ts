...
export async function GET(
  ...
  try {
    ...
    if (userId && registration.buyer_id !== userId) {
      return NextResponse.json(
        { success: false, error: 'Forbidden: you can only view your own tickets' },
        { status: 403 }
      );
    }

    const ticketCode = registration.ticket_code;
    const qrCodeUrl = registration.qr_code_url ?? `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/events/${registration.event_id}/ticket/${ticketCode}`;
