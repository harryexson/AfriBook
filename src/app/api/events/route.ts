...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!organizerId || !title || !description || !category || !startDate || !endDate) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: organizerId, title, description, category, startDate, endDate' },
        { status: 400 }
      );
    }
    ...
    if (subscription && subscription.max_events !== -1) {
      ...
      if ((count ?? 0) >= subscription.max_events) {
        return NextResponse.json(
          { success: false, error: 'Event limit reached for your subscription plan. Please upgrade.' },
          { status: 403 }
        );
      }
    }
    ...
    const feePercentMap: Record<string, number> = { free: 5, starter: 4, professional: 3, enterprise: 2 };
    const feeFixedMap: Record<string, number> = { free: 1, starter: 0.75, professional: 0.5, enterprise: 0.25 };

    let minPrice = 0;
    let maxPrice = 0;
    const isFree = !ticketTiers || ticketTiers.length === 0 || ticketTiers.every((t: { price?: number }) => (t.price ?? 0) === 0);

    if (ticketTiers && ticketTiers.length > 0) {
      const prices = ticketTiers.map((t: { price: number }) => t.price).filter((p: number) => p > 0);
    ...
    if (ticketTiers && ticketTiers.length > 0) {
      const tierRows = ticketTiers.map((tier: Record<string, unknown>, index: number) => ({
        event_id: event.id,
        name: tier.name,
      ...
      if (ttError) {
        return NextResponse.json(
          { success: false, error: 'Event created but failed to save ticket tiers' },
          { status: 500 }
        );
      }
    }
    ...
    return NextResponse.json(
      { success: true, data: fullEvent ?? event, message: 'Event created successfully' },
      { status: 201 }
    );
  } catch (error) {
    return NextResponse.json(
