...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, plan, billingPeriod, paymentMethodId } = body;

    if (!userId || !plan) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: userId, plan' },
        { status: 400 }
      );
    }
    ...
    const planPrices: Record<string, { monthly: number; annual: number; monthlyStripe: string; annualStripe: string }> = {
      starter: { monthly: 5000, annual: 50000, monthlyStripe: process.env.STRIPE_PRICE_STARTER_MONTHLY!, annualStripe: process.env.STRIPE_PRICE_STARTER_ANNUAL! },
      professional: { monthly: 15000, annual: 150000, monthlyStripe: process.env.STRIPE_PRICE_PROFESSIONAL_MONTHLY!, annualStripe: process.env.STRIPE_PRICE_PROFESSIONAL_ANNUAL! },
      enterprise: { monthly: 50000, annual: 500000, monthlyStripe: process.env.STRIPE_PRICE_ENTERPRISE_MONTHLY!, annualStripe: process.env.STRIPE_PRICE_ENTERPRISE_ANNUAL! },
    };
    ...
    const priceId = isAnnual ? priceConfig.annualStripe : priceConfig.monthlyStripe;
    ...
    if (existing?.stripe_subscription_id) {
      subscriptionParams.metadata = { ...subscriptionParams.metadata, replaced_subscription: existing.stripe_subscription_id };
    }

    const stripeSubscription = await stripe.subscriptions.create(subscriptionParams);

    const planLimits: Record<string, { maxEvents: number; maxTickets: number; maxGuests: number; commission: number; feeFixed: number }> = {
      starter: { maxEvents: 10, maxTickets: 500, maxGuests: 5, commission: 4, feeFixed: 0.75 },
      professional: { maxEvents: 50, maxTickets: 5000, maxGuests: 10, commission: 3, feeFixed: 0.5 },
      enterprise: { maxEvents: -1, maxTickets: -1, maxGuests: 20, commission: 2, feeFixed: 0.25 },
    };
