...
export async function POST(
  ...
  try {
    ...
    const { registrationId, guests } = body;
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Missing required fields: registrationId, guests[]",
        },
        { status: 400 },
      );
    }
    ...
    if (registration.buyer_id !== user.id) {
      return NextResponse.json(
        {
          success: false,
          error: "Forbidden: you can only add guests to your own registration",
        },
        { status: 403 },
      );
    }
    ...
    const maxGuests =
      (ticketType.max_guests_per_ticket ?? 0) * registration.quantity;
    ...
    if (guests.length > remainingSlots) {
      return NextResponse.json(
        {
          success: false,
          error: `Maximum ${maxGuests} guests allowed. ${remainingSlots} slots remaining.`,
        },
        { status: 400 },
      );
    }
  ...
  try {
    ...
    if (event.organizer_id !== user.id && !isAdmin) {
      return NextResponse.json(
        {
          success: false,
          error: "Unauthorized: only the organizer can view the guest list",
        },
        { status: 403 },
      );
    }
    ...
    const {
      ...
      .from("event_guests")
      .select(
        "*, ticket_purchases(buyer_name, buyer_email, event_ticket_types(name))",
        { count: "exact" },
      )
      .eq("event_id", eventId)
