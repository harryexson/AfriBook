...
export async function GET(
  ...
  try {
    ...
    const { data: dailySales } = await supabase
      .rpc('get_event_daily_sales' as never, {
        p_event_id: eventId,
        p_since_date: sinceDateStr,
      } as never);

    const dailySalesChart = (dailySales as { date: string; count: number; revenue: number }[] | null) ?? [];
    ...
    const conversionRate = event.view_count > 0
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        overview: {
          ...
          capacityUsedPercent: event.total_capacity > 0
        ...
        dailySales: dailySalesChart.sort((a, b) => a.date.localeCompare(b.date)),
