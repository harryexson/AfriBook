...
export async function POST(
  ...
  try {
    ...
    const { data: event, error: eventError } = await supabase
      .from('events')
      .select('id, title, slug, share_url, description, referral_code, enable_referrals')
      .eq('id', eventId)
      .single();
    ...
    const shareLinks: { platform: string; url: string; referralUrl?: string }[] = [];
    ...
    for (const platform of platforms) {
      ...
      const referralUrl = event.enable_referrals && event.referral_code
...
export async function GET(
  ...
  try {
    ...
    if (!organizerId || event.organizer_id !== organizerId) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized: only the organizer can view share stats' },
        { status: 403 }
      );
    }
