...
import type { TicketPurchase, EventGuest } from '@/types/events';
  ...
  try {
    ...
    if (!ticketTypeId || !quantity || !buyerName || !buyerEmail) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: ticketTypeId, quantity, buyerName, buyerEmail' },
        { status: 400 }
      );
    }
    ...
    if (quantity < (ticketType.min_per_order ?? 1)) {
      return NextResponse.json(
        { success: false, error: `Minimum order is ${ticketType.min_per_order} tickets` },
        { status: 400 }
      );
    }
    ...
    if (quantity > (ticketType.max_per_order ?? 10)) {
      return NextResponse.json(
        { success: false, error: `Maximum order is ${ticketType.max_per_order} tickets` },
        { status: 400 }
      );
    }
    ...
    if (quantity > available) {
      if (event.enable_waitlist) {
        return NextResponse.json(
          { success: false, error: 'Not enough tickets available. You can join the waitlist.', waitlistAvailable: true },
          { status: 409 }
        );
      }
      return NextResponse.json(
    ...
    if (guests && Array.isArray(guests) && guests.length > 0 && ticketType.includes_guest_registration) {
      ...
      createdGuests.push(...(insertedGuests as unknown as EventGuest[] ?? []));
    }
    ...
    if (buyerId) {
      await supabase.from('notifications').insert({
        ...
        data: { event_id: eventId, ticket_purchase_id: purchase.id, ticket_code: ticketCode },
      });
    }
...
export async function GET(
  ...
  try {
    ...
    } else if (organizerId && organizerId === event.organizer_id) {
      // Organizer sees all tickets for their event
    } else {
      return NextResponse.json(
        { success: false, error: 'Unauthorized: provide buyerId or organizerId' },
        { status: 403 }
      );
    }
