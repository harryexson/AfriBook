...
export async function POST(
  ...
  try {
    ...
    const { organizerId, code, discountType, discountValue, maxUses, validFrom, validUntil, minOrderAmount } = body;

    if (!organizerId || !code || !discountType || discountValue === undefined || !validFrom || !validUntil) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: organizerId, code, discountType, discountValue, validFrom, validUntil' },
        { status: 400 }
      );
    }
    ...
    if (event.organizer_id !== organizerId) {
      return NextResponse.json(
        { success: false, error: 'Forbidden: only the event organizer can create promo codes' },
        { status: 403 }
      );
    }
    ...
    if (existingPromo) {
      return NextResponse.json(
        { success: false, error: 'A promo code with this name already exists for this event' },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, data: promo, message: 'Promo code created successfully' },
      { status: 201 }
    );
  } catch (error) {
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    if (promo.used_count >= promo.max_uses) {
      return NextResponse.json(
        { success: false, error: 'Promo code has reached its maximum usage limit' },
        { status: 400 }
      );
    }
